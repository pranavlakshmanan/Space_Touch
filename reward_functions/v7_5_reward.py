#!/usr/bin/env python3
"""
V7.5 Per-Axis Derivative Reward Calculator

Key changes from V7.4.1:
- Per-axis overlap tracking (X, Y, Z independently)
- Rewards closing gaps on ANY axis, not just full 3D overlap
- Fixes the spawn position gap problem where X-axis has no initial overlap
- Provides dense gradient signal for navigation

Reward composition:
- PRIMARY: Per-axis derivative rewards (rewards closing gaps on each axis)
- SECONDARY: Full 3D overlap derivative (bonus when all axes overlap)
...
class V75RewardCalculator:
    ...
    def __init__(self, config: Dict = None):
        ...
        # V7.5: Per-axis reward parameters
        self.axis_improvement_scale = config.get('axis_improvement_scale', 500.0)  # Per cm of axis progress
        self.axis_regression_multiplier = config.get('axis_regression_multiplier', 1.5)

        # V7.5: Full overlap bonus (when all 3 axes overlap)
        self.full_overlap_bonus_scale = config.get('full_overlap_bonus_scale', 50000.0)
