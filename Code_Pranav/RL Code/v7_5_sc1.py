#!/usr/bin/env python3
"""
V7.5_SC1.py - Per-Axis Derivative Reward Training

Key changes from V7.4:
- Uses per-axis derivative rewards (fixes spawn position gap problem)
- Rewards progress on X, Y, Z axes independently
- Full 3D overlap bonus when all axes aligned
- Fixes step_counter reset bug
...
Usage:
    python v7_5_sc1.py train --timesteps 50000    # Train new model
    python v7_5_sc1.py --resume PATH              # Resume from checkpoint
"""
...
class V75Environment(VecEnv):
    """
    V7.5 Soft-Capture Environment with Per-Axis Derivative Reward

    Based on V7.4 with key change:
    - Per-axis derivative rewards (X, Y, Z independently)
    - Full 3D overlap bonus when all axes aligned
    - Fixes spawn position gap problem
    ...
    def __init__(self, 
        ...
        # Initialize V7.5 per-axis derivative reward calculator
        self.reward_calc = V75RewardCalculator({
            'object_radius': 0.05,
            'safety_margin': 0.025,
            'object_hull_points': 32,
            'hull_compute_freq': 12,  # 20Hz at 240Hz sim
            # V7.5 per-axis parameters
            'axis_improvement_scale': 500.0,
            'axis_regression_multiplier': 1.5,
            'full_overlap_bonus_scale': 50000.0,
...
class V75WandBCallback(BaseCallback):
    """V7.5 Lightweight WandB logging for per-axis derivative reward system"""
...
def train(args):
    """Main V7.5 training function - Per-axis derivative rewards"""
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    run_name = f"V7.5_SC1_{timestamp}"
    ...
    wandb.init(
        ...
        config={
            'version': 'V7.5',
            'experiment': 'per_axis_derivative_rewards',
            'total_timesteps': args.timesteps,
            'algorithm': 'PPO',
            'observation_dim': 32,
            'action_dim': 10,
            # V7.5 reward parameters
            'reward_type': 'per_axis_derivative',
            'axis_improvement_scale': 500.0,
            'axis_regression_multiplier': 1.5,
            'full_overlap_bonus_scale': 50000.0,
            'distance_reward_scale': 5.0,
        ...
        tags=['v7.5', 'soft-capture', 'per-axis-derivative', 'spawn-independent'],
    )

    print(f"Starting V7.5 training: {run_name}")
    print(f"  Total steps: {args.timesteps:,}")
    print(f"  Reward system: PER-AXIS DERIVATIVE (spawn-position independent)")
    print(f"  PRIMARY: Per-axis gap closing rewards")
    print(f"  SECONDARY: Full 3D overlap bonus")
    print(f"  TERTIARY: Distance reward (weak guide)")
    print(f"  Contact penalty: distance-scaled")
    print(f"  Log dir: {log_dir}")

    env = V75Environment(vis=args.vis, max_steps=500)
    ...
    # V7.5: No curriculum callback
    callbacks = [
        V75WandBCallback(log_freq=480, verbose=0),
        CheckpointCallback(
            save_freq=25000,  # V7.5: Checkpoint every 25K for 50K runs
            save_path=str(checkpoint_dir),
            name_prefix="v7_5_sc1",
...
def test(args):
    ...
    print(f"="*60)
    print(f"V7.5 SC-1 COMPREHENSIVE TESTING (Per-Axis Derivative)")
    print(f"="*60)
    ...
    # Load model
    env = V75Environment(vis=args.vis, max_steps=1000)
