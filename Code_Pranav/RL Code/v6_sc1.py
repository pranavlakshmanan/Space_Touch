#!/usr/bin/env python3
"""
V6_SC1.py - Clean Space Debris Soft-Capture Training

Key improvements over V5:
- Minimal print statements (no spam)
- Proper checkpoint saving
- Clean 3-phase curriculum: Proximity → Overlap → Precision Clearance
- All V5 fixes retained (HSI overlap, 9-point hull, 32-point object)
- FIXED: Hand-finger synchronization via direct position control

Usage:
    python v6_sc1.py                    # Train new model
    python v6_sc1.py --resume PATH      # Resume from checkpoint
    python v6_sc1.py --test PATH        # Test trained model
"""
...
# Import V6 reward calculator
sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
from reward_functions.v6_reward import V6RewardCalculator
...
class V6Environment(VecEnv):
    """
    V6 Soft-Capture Environment
    
    CRITICAL FIX: Uses direct position control (resetBasePositionAndOrientation)
    instead of velocity control to ensure fingers stay synchronized with hand base.
    """
    ...
    def __init__(self, 
        ...
        # Initialize reward calculator with V5 fixes + MEMORY FIX
        self.reward_calc = V6RewardCalculator({
            'object_radius': 0.05,
            'safety_margin': 0.025,
            'object_hull_points': 32,
            'hull_compute_freq': 24,  # Compute hulls at 10Hz instead of 240Hz
        })
...
class V6CurriculumCallback(BaseCallback):
    """Performance-based automatic curriculum progression"""
    ...
    def __init__(self, check_freq=10000, verbose=0):
        ...
        # TUNED: Performance-based thresholds (not time-based!)
        self.phase_thresholds = {
            0: {  # Phase 0: Ultra-close learning
                'min_steps': 50000,          # At least 50K steps in phase
                'max_steps': 200000,         # Force advance after 200K if stuck
                'mean_distance': 0.12,       # Average distance < 12cm
                'mean_overlap': 0.000003,    # Average overlap > 3 cm³
                'window_size': 500,          # Average over last 500 steps
            },
            1: {  # Phase 1: Proximity + Low Overlap
                'min_steps': 100000,         # At least 100K steps
                'max_steps': 400000,         # Force advance after 400K
                'mean_distance': 0.15,       # Average distance < 15cm
                'mean_overlap': 0.000005,    # Average overlap > 5 cm³
                'window_size': 1000,
            },
            2: {  # Phase 2: Maximize Overlap
                'min_steps': 150000,
                'max_steps': 500000,
                'mean_overlap': 0.000015,    # Average overlap > 15 cm³
                'mean_distance': 0.12,       # Stay close
                'window_size': 1000,
            },
            3: {  # Phase 3: Final phase (no advancement)
                'min_steps': float('inf'),
                'max_steps': float('inf'),
            },
        }
...
def train(args):
    ...
    wandb.init(
        ...
        config={
            ...
            'overlap_method': 'half_space_intersection',
        },
        tags=['v6', 'soft-capture', 'position-control', 'hsi-overlap'],
    )
    
    print(f"Starting V6 training: {run_name}")
    print(f"  Total steps: {args.timesteps:,}")
    print(f"  Log dir: {log_dir}")
    
    env = V6Environment(vis=args.vis, max_steps=500)
    ...
    callbacks = [
        V6CurriculumCallback(check_freq=10000, verbose=0),
...
def test(args):
    ...
    # Load model
    env = V6Environment(vis=args.vis, max_steps=1000)
...
def main():
    parser = argparse.ArgumentParser(description='V6 SC-1 Soft-Capture Training')
    subparsers = parser.add_subparsers(dest='command', help='Command to run')
    
    train_parser = subparsers.add_parser('train', help='Train a new model')
    train_parser.add_argument('--timesteps', type=int, default=500000, help='Total training steps')
