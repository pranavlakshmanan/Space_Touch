...
class TendonController:
    ...
    def __init__(self, hand_id, joint_names, joint_indices):
        ...
        self.TENDON_FORCE_GAIN = 15.0   # Base gain for tendon force to torque conversion
        self.TENDON_DAMPING = 0.8       # Damping factor for stability
        self.MAX_TENDON_FORCE = 60.0    # Maximum tendon force (N)
    ...
    def _on_step(self) -> bool:
        ...
        if self.locals.get('infos'):
            for info in self.locals['infos']:
                ...
                # Step-wise metrics logging (for TensorBoard only, not data logger)
                if 'distance' in info:
                ...
                # Don't log step data during training - only episode completion for TensorBoard
...
class TendonAllegroReachingEnv(VecEnv):
    ...
    def _setup_simulation(self):
        ...
        try:
            ...
            # Initialize tactile sensor with GL context fallback
            try:
                
                raise Exception("Temporarily disabled for testing")
                bg = cv2.imread("/home/pralak/Space_Touch/examples/conf/bg_digit_240_320.jpg")
                self.tactile_sensor = tacto.Sensor(120, 160,
                                                 background=bg,
                                                 config_path="/home/pralak/Space_Touch/tacto/config_digit.yml")
                
                # Add cameras to fingertips
                tip_labels = {
                ...
                for i in range(num_joints):
                    ...
                    if joint_name.endswith("_tip") and joint_name in tip_labels:
                        self.tactile_sensor.add_camera(self.hand, [i])
                        
                        # Add a small sphere for contact
                        pos = p.getLinkState(self.hand, i)[0]
                        sphere_urdf = "/home/pralak/Space_Touch/examples/objects/sphere_small.urdf"
                        if os.path.exists(sphere_urdf):
                            sphere = p.loadURDF(sphere_urdf, 
                                              [pos[0], pos[1], pos[2]+0.05],
                                              globalScaling=0.1)
                            self.tactile_sensor.add_body(sphere)
                ...
                if not hasattr(self, '_tactile_error_printed'):
                    print(f"⚠ Tactile sensor disabled (GL context issue in headless mode): Training will continue without tactile feedback")
    ...
    def step_wait(self):
        ...
        try:
            ...
            # Apply base movement (reduced for smoother motion)
            ...
            tendon_efficiency = 1.0 - 0.5 * np.mean(tendon_forces)  # Reward for not over-actuating
            tendon_efficiency_reward = 0.2 * tendon_efficiency
            
            # 3. Tactile contact reward (encourage contact with target)
            tactile_contact_reward = 0.1 * np.sum(tactile_spans > 0.05)  # Reward contact detection
            ...
            # Store data for potential logging during testing (but don't log during training)
            self.step_data = {
                ...
                "tactile_contact_detected": int(np.any(tactile_spans > 0.05)),
                "reward": total_reward,
            ...
            # Only log detailed data if data_logger is active (during testing)
            if self.data_logger is not None:
                self.data_logger.log_step(self.step_data)
            
            # Minimal info for training (for TensorBoard)
            infos = [{
                "success": success,
                "distance": distance,
                "distance_reward": distance_reward,
...
def create_plots(csv_file):
    """Create comprehensive plots from the logged data with tendon and tactile analysis"""
    ...
    try:
        ...
        fig, axes = plt.subplots(2, 2, figsize=(15, 10))
        fig.suptitle('Tendon Force Analysis', fontsize=16)
        
        # Individual tendon forces
        axes[0, 0].plot(step_vals, df_plot['tendon_force_index'].values, label='Index', alpha=0.8)
        axes[0, 0].plot(step_vals, df_plot['tendon_force_middle'].values, label='Middle', alpha=0.8)
        axes[0, 0].plot(step_vals, df_plot['tendon_force_ring'].values, label='Ring', alpha=0.8)
        axes[0, 0].plot(step_vals, df_plot['tendon_force_thumb'].values, label='Thumb', alpha=0.8)
        ...
        axes[1, 0].hist([df_plot['tendon_force_index'].values, df_plot['tendon_force_middle'].values,
                       ...
                       bins=50, alpha=0.7, label=['Index', 'Middle', 'Ring', 'Thumb'])
        ...
        # Tendon coordination (correlation between fingers)
        tendon_correlation = np.corrcoef([df_plot['tendon_force_index'].values, 
                                        df_plot['tendon_force_middle'].values,
                                        df_plot['tendon_force_ring'].values, 
                                        df_plot['tendon_force_thumb'].values])
        im = axes[1, 1].imshow(tendon_correlation, cmap='coolwarm', vmin=-1, vmax=1)
        axes[1, 1].set_title('Tendon Force Correlation')
        axes[1, 1].set_xticks(range(4))
        axes[1, 1].set_yticks(range(4))
        axes[1, 1].set_xticklabels(['Index', 'Middle', 'Ring', 'Thumb'])
        axes[1, 1].set_yticklabels(['Index', 'Middle', 'Ring', 'Thumb'])
        plt.colorbar(im, ax=axes[1, 1])
        
        plt.tight_layout()
        plt.savefig(plot_dir / 'tendon_force_analysis.png', dpi=150, bbox_inches='tight')
        plt.close()
        print("✓ Tendon force analysis plot created")
        ...
        # Tendon efficiency reward
        axes[1, 0].plot(step_vals, df_plot['tendon_efficiency_reward'].values, alpha=0.8, linewidth=0.8)
        axes[1, 0].set_title('Tendon Efficiency Reward')
        axes[1, 0].set_xlabel('Step')
        axes[1, 0].set_ylabel('Tendon Efficiency Reward')
        axes[1, 0].grid(True, alpha=0.3)
        ...
        # 4. Tactile Sensor Analysis
        fig, axes = plt.subplots(2, 2, figsize=(15, 10))
        fig.suptitle('Tactile Sensor Analysis', fontsize=16)
        
        # Tactile deformation over time
        axes[0, 0].plot(step_vals, df_plot['tactile_deformation_spans'].values, alpha=0.8, linewidth=0.8)
        axes[0, 0].set_title('Tactile Deformation Spans')
        axes[0, 0].set_xlabel('Step')
        axes[0, 0].set_ylabel('Deformation')
        axes[0, 0].grid(True, alpha=0.3)
        
        # Contact detection over time
        axes[0, 1].plot(step_vals, df_plot['tactile_contact_detected'].values, alpha=0.8, linewidth=0.8)
        axes[0, 1].set_title('Contact Detection')
        axes[0, 1].set_xlabel('Step')
        axes[0, 1].set_ylabel('Contact Detected (0/1)')
        axes[0, 1].grid(True, alpha=0.3)
        
        # Tactile vs distance relationship
        axes[1, 0].scatter(df_plot['distance_to_target'].values, df_plot['tactile_deformation_spans'].values, 
                          alpha=0.5, s=1)
        axes[1, 0].set_title('Tactile Deformation vs Distance to Target')
        axes[1, 0].set_xlabel('Distance to Target (m)')
        axes[1, 0].set_ylabel('Tactile Deformation')
        axes[1, 0].grid(True, alpha=0.3)
        
        # Contact frequency per episode
        if 'episode' in df.columns:
            try:
                episode_contact = df.groupby('episode')['tactile_contact_detected'].mean()
                ...
                axes[1, 1].set_title('Contact Frequency per Episode')
                axes[1, 1].set_xlabel('Episode')
                axes[1, 1].set_ylabel('Contact Frequency')
                axes[1, 1].grid(True, alpha=0.3)
            except Exception as e:
                print(f"Skipping episode contact plot due to error: {e}")
                axes[1, 1].text(0.5, 0.5, 'Episode data unavailable', ha='center', va='center', transform=axes[1, 1].transAxes)
    ...
    if len(df) > 0:
        ...
        print(f"Contact detection rate: {df['tactile_contact_detected'].mean()*100:.1f}%")
...
if __name__ == "__main__":
    ...
    log_dir = base_training_dir / f"Run_{timestamp}_SC1_TendonControl"
    log_dir.mkdir(parents=True, exist_ok=True)
    
    print("=" * 80)
    print(f"SC-1 TENDON-BASED ALLEGRO HAND TRAINING - RUN #{run_number}")
    print("=" * 80)
    ...
    try:
        # Create data logger (but don't attach to training env yet)
        data_logger = DataLogger(log_dir)
        
        # Create environment with tendon control
        print("\nCreating tendon-controlled reaching environment...")
        env = TendonAllegroReachingEnv(vis=False)  # Always headless
        # Don't set data logger for training environment (no detailed logging during training)
        ...
        # Create PPO model optimized for tendon control with GPU acceleration
        ...
        model = PPO(
            "MlpPolicy",  # MLP for enhanced observations
            env, 
            verbose=1,
            ...
            vf_coef=0.5,
            max_grad_norm=0.5,
            device=device  # Use detected device (GPU if available)
        )
        ...
        print("Observation Space: 30D (base + target + fingers + tactile + tendons)")
        print("Reward Components:")
        print("  - Distance reward (exponential)")
        print("  - Tendon efficiency reward") 
        print("  - Tactile contact reward")
        print("  - Movement efficiency penalty")
        print("  - Action smoothness penalty")
        print("  - Success bonus")
        print("\nLOGGING CONFIGURATION:")
        print("  - Training: TensorBoard metrics only (lightweight)")
        print("  - Testing: Full detailed data logging enabled")
        print("=" * 80)
        ...
        # Train the model
        print("Starting SC-1 tendon-based training...")
        model.learn(
            total_timesteps=100000,  # Extended training for complex control
            callback=callback,
            log_interval=10,
            tb_log_name=f"SC1_TendonControl_run{run_number}",
            reset_num_timesteps=True,
            progress_bar=True
        )
        
        # Save the model to the training run directory
        model_path = log_dir / f"sc1_tendon_model_run{run_number}_{timestamp}"
        model.save(str(model_path))
        print(f"\n✓ Training Complete! Model saved to {model_path}.zip")
        
        # No training data to save (logging only happens during testing)
        
        # Test the trained model with detailed logging
        print(f"\nTesting trained tendon control model for 10 episodes...")
        print("=" * 60)
        print("TESTING PHASE EXPLANATION:")
        print("Testing the trained tendon-based control policy")
        print("to evaluate performance on:")
        print("  1. Reaching accuracy with combined base+tendon control")
        print("  2. Tendon coordination and efficiency")
        print("  3. Tactile feedback utilization")
        print("  4. Overall task success rate")
        print("NOTE: Detailed data logging is ENABLED for testing phase")
        print("=" * 60)
        
        # Create separate data logger for testing
        test_data_logger = DataLogger(log_dir / "test_data")
        
        env_test = TendonAllegroReachingEnv(vis=False)  # Create fresh env for testing
        env_test.set_data_logger(test_data_logger)  # Enable detailed logging for testing
        ...
        while test_episode < 10 and test_steps < 5000:
            ...
            if done[0]:
                test_episode += 1
                test_data_logger.new_episode()  # Track episode changes in test data
                ...
                print(f"  Test Episode {test_episode:2d}: {status} | Distance={distance:.4f}m | Reward={episode_reward:.1f} | Avg Tendon={avg_tendon:.3f}")
                obs = env_test.reset()
        ...
        # Generate analysis plots from test data
        print(f"\nGenerating comprehensive analysis plots from test data...")
        create_plots(test_csv)
        
        print("\n" + "=" * 80)
        print("SC-1 TENDON TRAINING COMPLETE!")
        print("=" * 80)
        print(f"All results saved to: {log_dir}")
        print(f"TensorBoard logs (training): {tb_log_dir}")
        print(f"Test data: {log_dir}/test_data/")
        print(f"Analysis plots (from test): {log_dir}/test_data/plots/")
        ...
        print("Note: Detailed data logging only occurs during testing phase.")
        print("Model saved if training completed any episodes.")
