...
import torch
# Force CPU usage
torch.set_num_threads(4)  # Optimize for CPU performance
device = torch.device("cpu")
print("CPU-only mode enabled - no GPU acceleration")
...
class CPUMonitor:
    ...
    def __init__(self):
        self.max_memory_threshold = 8.0  # 8GB RAM warning threshold
    ...
    def check_system_status(self):
        ...
        if memory_gb > self.max_memory_threshold:
            print(f"WARNING: High memory usage: {memory_gb:.1f}GB")
            
        return True, memory_gb, cpu_percent
...
class DataLogger:
    ...
    def log_step(self, data_dict):
        ...
        import psutil
        self.data['memory_used_gb'].append(psutil.virtual_memory().used / 1e9)
        self.data['cpu_percent'].append(psutil.cpu_percent(interval=None))
...
class TendonController:
    ...
    def __init__(self, hand_id, joint_names, joint_indices):
        ...
        self.TENDON_FORCE_GAIN = 10.0
        self.TENDON_DAMPING = 1.2
        self.MAX_TENDON_FORCE = 40.0
...
class EnhancedTensorboardCallback(BaseCallback):
    """Enhanced callback for comprehensive TensorBoard logging with CPU monitoring"""
    
    def __init__(self, data_logger, cpu_monitor, log_freq=200, verbose=0):
    ...
    def _on_step(self) -> bool:
        # CPU monitoring
        system_ok, memory_gb, cpu_percent = self.cpu_monitor.check_system_status()
        ...
        if self.locals.get('infos'):
            for info in self.locals['infos']:
                ...
                if 'episode' in info:
                    ...
                    self.logger.record('system/memory_gb', memory_gb)
        ...
        if self.num_timesteps % self.log_freq == 0:
            ...
            # Training progress and system monitoring
            self.logger.record('training/timesteps', self.num_timesteps)
            self.logger.record('training/episodes', len(self.episode_rewards))
            self.logger.record('system/memory_current_gb', memory_gb)
            self.logger.record('system/cpu_current_percent', cpu_percent)
...
class TendonAllegroReachingEnv(VecEnv):
    ...
    def _setup_simulation(self):
        ...
        try:
            ...
            # Initialize tactile sensor with simplified setup
            try:
                # Disable tactile sensor for CPU performance
                self.tactile_sensor = None
                if not hasattr(self, '_tactile_disabled_printed'):
                    print("Tactile sensor disabled for CPU performance")
    ...
    def _get_tactile_feedback(self):
        """Get tactile sensor feedback - simplified"""
        # Return zeros since tactile is disabled for CPU performance
    ...
    def step_wait(self):
        ...
        try:
            ...
            # Apply base movement
            linear_vel = base_actions[:3] * 0.2
            angular_vel = base_actions[3:6] * 0.5
            ...
            movement_penalty = -0.005 * (linear_vel_magnitude + angular_vel_magnitude)
            
            # 5. Action smoothness penalty
            action_diff = np.linalg.norm(actions - self.prev_actions[0])
            smoothness_penalty = -0.02 * action_diff
            
            # 6. Success bonus
            success_bonus = 5.0 if distance < 0.1 else 0.0
            ...
            infos = [{
                ...
                "tactile_contact_detected": 0,  # Disabled for CPU performance
                "reward": total_reward,
                "tendon_efficiency_reward": tendon_efficiency_reward,
            }]
...
def create_plots(csv_file):
    """Create comprehensive plots from the logged data with CPU monitoring"""
    ...
    try:
        ...
        if 'memory_used_gb' in df_plot.columns and 'cpu_percent' in df_plot.columns:
            axes[1, 0].plot(step_vals, df_plot['memory_used_gb'].values, 'green', linewidth=1.0, label='Memory (GB)')
            ax_cpu = axes[1, 0].twinx()
            ax_cpu.plot(step_vals, df_plot['cpu_percent'].values, 'orange', linewidth=1.0, label='CPU (%)')
            axes[1, 0].set_title('System Resource Usage')
            axes[1, 0].set_xlabel('Step')
            axes[1, 0].set_ylabel('Memory (GB)', color='green')
            ax_cpu.set_ylabel('CPU (%)', color='orange')
        ...
        axes[1, 0].hist([df_plot['tendon_force_index'].values, df_plot['tendon_force_middle'].values,
                       ...
                       bins=30, alpha=0.7, label=['Index', 'Middle', 'Ring', 'Thumb'])
        ...
        if 'episode' in df_plot.columns and 'success' in df_plot.columns:
            try:
                ...
                if len(episode_success) > 10:
                    window = min(25, len(episode_success) // 10)
    ...
    print("\n=== SC-1 CPU-ONLY TRAINING SUMMARY ===")
    ...
    if len(df) > 0:
        ...
        if 'memory_used_gb' in df.columns:
            max_memory = df['memory_used_gb'].max()
            avg_memory = df['memory_used_gb'].mean()
...
if __name__ == "__main__":
    ...
    base_training_dir = Path("./SC1_Training_Runs_CPU/")
    base_training_dir.mkdir(parents=True, exist_ok=True)
    
    log_dir = base_training_dir / f"Run_{timestamp}_SC1_CPU_Only"
    log_dir.mkdir(parents=True, exist_ok=True)
    
    print("=" * 80)
    print(f"SC-1 CPU-ONLY TENDON-BASED TRAINING - RUN #{run_number}")
    print("=" * 80)
    ...
    try:
        ...
        # Create environment
        print("\nCreating CPU-only tendon-controlled environment...")
        env = TendonAllegroReachingEnv(vis=False)
        env.set_data_logger(data_logger)
        
        # Create enhanced callback for TensorBoard logging with CPU monitoring
        callback = EnhancedTensorboardCallback(data_logger, cpu_monitor, log_freq=200)
        
        # Configure custom logger for TensorBoard
        custom_logger = configure(str(tb_log_dir), ["stdout", "tensorboard"])
        
        # Create PPO model for CPU
        print("Creating CPU-only PPO model...")
        ...
        print("\n" + "=" * 80)
        print("SC-1 CPU-ONLY TRAINING CONFIGURATION")
        print("=" * 80)
        ...
        print(f"Simulation Frequency: {env.sim_freq} Hz")
        print("CPU-Only Features:")
        print("  - All computation on CPU")
        print("  - Optimized for CPU threading")
        print("  - Tactile sensors disabled for performance") 
        ...
        # Train the model
        print("Starting SC-1 CPU-only training...")
        model.learn(
            total_timesteps=50000,
            callback=callback,
            log_interval=20,
            tb_log_name=f"SC1_CPU_run{run_number}",
            reset_num_timesteps=True,
            progress_bar=True
        )
        
        # Save the model
        model_path = log_dir / f"sc1_cpu_model_run{run_number}_{timestamp}"
        model.save(str(model_path))
        print(f"\nTraining Complete! Model saved to {model_path}.zip")
        
        # Save training data to CSV
        csv_file = data_logger.save_to_csv("sc1_cpu_training_data.csv")
        
        # Test the trained model
        print(f"\nTesting trained CPU-only model for 5 episodes...")
        print("=" * 60)
        ...
        while test_episode < 5 and test_steps < 2000:
        ...
        print("\n" + "=" * 80)
        print("SC-1 CPU-ONLY TEST RESULTS")
        print("=" * 80)
        ...
        print("\n" + "=" * 80)
        print("SC-1 CPU-ONLY TRAINING COMPLETE!")
        print("=" * 80)
