#!/usr/bin/env python3
"""
V7.6_SC1.py - Curriculum-Based Palm Distance Reward Training

Key changes from V7.5.1:
- PRIMARY: Palm-to-target distance derivative (stable, not finger-dependent)
- SECONDARY: Bbox overlap derivative (only when distance < 8cm)
- 3-phase curriculum: 3.6cm → 5.8cm → 11.2cm spawn distance
- REMOVED: Memory optimization (compute overlap every step)
- Contact penalty (unchanged)

Reward composition:
- Distance derivative (always active)
- Overlap derivative (activates when close)
- Sustain bonus (for maintaining overlap)
- Contact penalty

Curriculum phases:
- Phase 1: Spawn 3.6cm from target (overlap possible immediately)
- Phase 2: Spawn 5.8cm from target (overlap possible immediately)
- Phase 3: Spawn 11.2cm from target (must approach for overlap)

Default training: 50K timesteps

Observation space: 32D (unchanged from V7.4)
- Smoothed overlap volume (1D)
- Relative distance vector (3D)

Usage:
    python v7_6_sc1.py train --timesteps 50000    # Train new model
    python v7_6_sc1.py --resume PATH              # Resume from checkpoint
"""
...
class V76Environment(VecEnv):
    """
    V7.6 Soft-Capture Environment with Curriculum-Based Palm Distance Reward

    Based on V7.5.1 with key changes:
    - PRIMARY: Palm-to-target distance derivative (stable gradient)
    - SECONDARY: Bbox overlap derivative (when close)
    - 3-phase curriculum with spawn distance progression
    - Compute overlap every step (no memory optimization)
    - 32D observation space (unchanged)
    """
    ...
    def __init__(self, 
        ...
        # Initialize V7.6 curriculum-based reward calculator
        self.reward_calc = V76RewardCalculator({
            'object_radius': 0.05,
            'safety_margin': 0.025,
            ...
            'overlap_regression_scale': 10000.0,
            # Sustain bonus
    ...
    def _spawn_hand(self):
        """Spawn Allegro hand at curriculum-dependent position"""
        if self.hand_id is not None:
            p.removeBody(self.hand_id)

        # V7.6: Get spawn offset from reward calculator (curriculum-dependent)
        offset = self.reward_calc.get_spawn_offset()
    ...
    def update_phase(self, phase: int):
        """V7.6: Update curriculum phase"""
        self.reward_calc.update_phase(phase)

    def update_reward_phase(self, phase: int):
        """Update curriculum phase (called by curriculum callback)"""
...
class V76CurriculumCallback(BaseCallback):
    """Curriculum progression based on success rate"""

    def __init__(self,
                 phase1_threshold=10000,   # Steps before considering phase 2
                 phase2_threshold=30000,   # Steps before considering phase 3
                 success_rate_threshold=0.3,  # Required success rate to advance
                 eval_window=1000,
                 verbose=0):
    ...
    def _on_step(self) -> bool:
        ...
        if infos and 'reward_info' in infos[0]:
            is_success = infos[0]['reward_info'].get('is_success', False)
            self.recent_successes.append(1 if is_success else 0)

            # Keep window size limited
            if len(self.recent_successes) > self.eval_window:
                self.recent_successes = self.recent_successes[-self.eval_window:]

        # Check for phase advancement
        if len(self.recent_successes) >= self.eval_window:
            success_rate = np.mean(self.recent_successes)

            if self.current_phase == 1 and self.num_timesteps >= self.phase1_threshold:
                if success_rate >= self.success_rate_threshold:
                    self.current_phase = 2
                    self.training_env.env_method('update_reward_phase', 2)
                    self.recent_successes = []  # Reset for new phase
                    if wandb.run:
                        wandb.log({'curriculum/phase': 2, 'curriculum/phase_change_step': self.num_timesteps})
                    print(f"\n>>> Phase 1 → 2 at step {self.num_timesteps} (success rate: {success_rate:.1%})")

            elif self.current_phase == 2 and self.num_timesteps >= self.phase2_threshold:
                if success_rate >= self.success_rate_threshold:
                    self.current_phase = 3
                    self.training_env.env_method('update_reward_phase', 3)
                    self.recent_successes = []
                    if wandb.run:
                        wandb.log({'curriculum/phase': 3, 'curriculum/phase_change_step': self.num_timesteps})
                    print(f"\n>>> Phase 2 → 3 at step {self.num_timesteps} (success rate: {success_rate:.1%})")

        return True


class V76WandBCallback(BaseCallback):
    """V7.6 Lightweight WandB logging for curriculum-based palm distance reward system"""
    ...
    def _log_metrics(self):
        ...
        log_dict = {
            ...
            # Rewards
            'reward/total': info.get('total_reward', 0),
            'reward/distance_derivative': info.get('distance_derivative_reward', 0),
            'reward/overlap_derivative': info.get('overlap_derivative_reward', 0),
            'reward/sustain_bonus': info.get('sustain_bonus', 0),
            'reward/contact_penalty': info.get('contact_penalty', 0),
            # State
            'state/palm_distance_cm': info.get('palm_to_target_distance', 0) * 100,
            'state/overlap_cm3': info.get('overlap_volume', 0) * 1e6,
            'state/contacts': info.get('num_contacts', 0),
            'state/consecutive_success': info.get('consecutive_success_steps', 0),
            # Per-axis (for debugging)
            'hull/axis_overlap_x_cm': info.get('axis_overlap_x', 0) * 100,
            'hull/axis_overlap_y_cm': info.get('axis_overlap_y', 0) * 100,
            'hull/axis_overlap_z_cm': info.get('axis_overlap_z', 0) * 100,
            # Curriculum
            'curriculum/phase': info.get('current_phase', 1),
        }
        ...
        if self.recent_distances_for_stats:
            log_dict['stats/mean_palm_distance_cm'] = np.mean(self.recent_distances_for_stats) * 100
...
def train(args):
    """Main V7.6 training function - Curriculum-based palm distance rewards"""
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    run_name = f"V7.6_SC1_{timestamp}"
    ...
    wandb.init(
        ...
        config={
            'version': 'V7.6',
            'experiment': 'curriculum_palm_distance_overlap',
            ...
            'phase3_spawn_distance_cm': 11.2,
            'phase1_threshold_steps': 10000,
            'phase2_threshold_steps': 30000,
            # Training
            'learning_rate': 3e-4,
            'n_steps': 4096,
            'ent_coef': 0.005,
        },
        tags=['v7.6', 'soft-capture', 'curriculum', 'palm-distance'],
    )

    print(f"Starting V7.6 training: {run_name}")
    print(f"  Total steps: {args.timesteps:,}")
    print(f"  Reward system: CURRICULUM + PALM DISTANCE + OVERLAP")
    print(f"  PRIMARY: Palm-to-target distance derivative")
    print(f"  SECONDARY: Bbox overlap derivative (when distance < 8cm)")
    print(f"  Curriculum: Phase 1 (3.6cm) → Phase 2 (5.8cm) → Phase 3 (11.2cm)")
    print(f"  Log dir: {log_dir}")

    env = V76Environment(vis=args.vis, max_steps=500)
    ...
    # V7.6: Curriculum callback + WandB logging
    callbacks = [
        V76CurriculumCallback(
            phase1_threshold=10000,
            phase2_threshold=30000,
            success_rate_threshold=0.3,
            eval_window=1000,
            verbose=0
        ),
        V76WandBCallback(log_freq=480, verbose=0),
        CheckpointCallback(
            save_freq=25000,  # V7.6: Checkpoint every 25K for 50K runs
            save_path=str(checkpoint_dir),
            name_prefix="v7_6_sc1",
...
def test(args):
    ...
    print(f"="*60)
    print(f"V7.6 SC-1 COMPREHENSIVE TESTING (Curriculum Palm Distance)")
    print(f"="*60)
    ...
    # Load model
    env = V76Environment(vis=args.vis, max_steps=1000)
