#!/usr/bin/env python3
"""
V7.5.1_SC1.py - Per-Axis Derivative Reward Training (Stabilized)

Key changes from V7.5:
- STABILIZED reward scaling (10x reduction)
- Symmetric penalties (regression_multiplier = 1.0)
- Added reward clipping (axis: ±10.0, total: ±20.0)
- Per-axis scale: 50.0 (was 500.0)
- Full overlap scale: 5000.0 (was 50000.0)

Reward composition:
- PRIMARY: Per-axis derivatives (closes gaps on any axis)
- SECONDARY: Full overlap bonus (when all 3 axes overlap)
- TERTIARY: Distance reward (weak guide)
...
Usage:
    python v7_5_1_sc1.py train --timesteps 50000    # Train new model
    python v7_5_1_sc1.py --resume PATH              # Resume from checkpoint
"""
...
# Import V7.5.1 reward calculator
sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
from reward_functions.v7_5_1_reward import V751RewardCalculator
...
class V751Environment(VecEnv):
    """
    V7.5.1 Soft-Capture Environment with Stabilized Per-Axis Derivative Reward

    Based on V7.5 with stabilized scaling:
    - Per-axis derivative rewards (X, Y, Z independently)
    - STABILIZED: 10x reduced scaling, symmetric penalties
    ...
    def __init__(self, 
        ...
        # Initialize V7.5.1 STABILIZED per-axis derivative reward calculator
        self.reward_calc = V751RewardCalculator({
            'object_radius': 0.05,
            'safety_margin': 0.025,
            'object_hull_points': 32,
            'hull_compute_freq': 12,  # 20Hz at 240Hz sim
            # V7.5.1 STABILIZED per-axis parameters (10x reduction)
            'axis_improvement_scale': 50.0,         # Reduced from 500.0
            'axis_regression_multiplier': 1.0,      # Symmetric (was 1.5)
            'full_overlap_bonus_scale': 5000.0,     # Reduced from 50000.0
            'sustain_threshold': 0.00005,  # 50 cm³
            'sustain_bonus': 2.0,
            # Distance reward (reduced weight)
...
class V751WandBCallback(BaseCallback):
    """V7.5.1 Lightweight WandB logging for stabilized per-axis derivative reward system"""
    ...
    def _log_metrics(self):
        ...
        # V7.5: Total reward from per-axis + full overlap components
        total_reward = info.get('total_reward', 0)

        log_dict = {
            'train/step': self.num_timesteps,
            'reward/total': total_reward,
            # V7.5: Per-axis rewards
            'reward/axis_x': info.get('axis_x_reward', 0),
            'reward/axis_y': info.get('axis_y_reward', 0),
            'reward/axis_z': info.get('axis_z_reward', 0),
            'reward/total_axis': info.get('total_axis_reward', 0),
            'reward/full_overlap_bonus': info.get('full_overlap_bonus', 0),
            'reward/sustain_bonus': info.get('sustain_bonus', 0),
            'reward/distance': info.get('distance_reward', 0),
            'reward/contact_penalty': info.get('contact_penalty', 0),
            # V7.5: Per-axis overlap values (in cm)
...
def train(args):
    """Main V7.5.1 training function - Stabilized per-axis derivative rewards"""
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    run_name = f"V7.5.1_SC1_{timestamp}"
    ...
    wandb.init(
        ...
        config={
            'version': 'V7.5.1',
            'experiment': 'stabilized_per_axis_derivative_rewards',
            'total_timesteps': args.timesteps,
            'algorithm': 'PPO',
            'observation_dim': 32,
            'action_dim': 10,
            # V7.5.1 STABILIZED reward parameters
            'reward_type': 'per_axis_derivative_stabilized',
            'axis_improvement_scale': 50.0,         # Reduced from 500.0
            'axis_regression_multiplier': 1.0,      # Symmetric (was 1.5)
            'full_overlap_bonus_scale': 5000.0,     # Reduced from 50000.0
            'distance_reward_scale': 5.0,
            ...
            'sustain_bonus': 2.0,
            'axis_reward_clip': [-10.0, 10.0],
            'total_reward_clip': [-20.0, 20.0],
        ...
        tags=['v7.5.1', 'soft-capture', 'per-axis-derivative', 'stabilized'],
    )

    print(f"Starting V7.5.1 training: {run_name}")
    print(f"  Total steps: {args.timesteps:,}")
    print(f"  Reward system: STABILIZED PER-AXIS DERIVATIVE")
    print(f"  PRIMARY: Per-axis gap closing rewards (50.0 scale)")
    print(f"  SECONDARY: Full 3D overlap bonus (5000.0 scale)")
    print(f"  TERTIARY: Distance reward (weak guide)")
    print(f"  Reward clipping: axis ±10.0, total ±20.0")
    print(f"  Log dir: {log_dir}")

    env = V751Environment(vis=args.vis, max_steps=500)
    ...
    # V7.5.1: No curriculum callback
    callbacks = [
        V751WandBCallback(log_freq=480, verbose=0),
        CheckpointCallback(
            save_freq=25000,  # V7.5.1: Checkpoint every 25K for 50K runs
            save_path=str(checkpoint_dir),
            name_prefix="v7_5_1_sc1",
...
def test(args):
    ...
    print(f"="*60)
    print(f"V7.5.1 SC-1 COMPREHENSIVE TESTING (Stabilized Per-Axis Derivative)")
    print(f"="*60)
    ...
    # Load model
    env = V751Environment(vis=args.vis, max_steps=1000)
