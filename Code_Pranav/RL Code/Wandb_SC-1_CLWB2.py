...
class TendonAllegroReachingEnv(VecEnv):
    """Enhanced environment with delta-distance reward and adaptive episode length"""
    ...
    def __init__(self, 
        ...
        # MODIFIED: Observation without distance and tendon forces
        # Observation: base_pos(3) + target_pos(3) + base_vel(3) + finger_positions(12) + binary_tactile(4)
        obs_dim = 3 + 3 + 3 + 12 + 4  # = 25 (removed distance(1) and tendon_forces(4))
        ...
        # NEW: Track previous distance for delta reward calculation
        self.prev_distance = np.zeros(num_envs, dtype=np.float32)
        
        # NEW: Track initial distance for adaptive episode length
    ...
    def _init_pybullet(self):
        ...
        p.setTimeStep(1.0/self.sim_freq if hasattr(self, 'sim_freq') else 1.0/240)
    ...
    def _get_observation(self):
        """Get current observation - MODIFIED to exclude distance and tendon forces"""
        try:
            if self.hand is None:
            ...
            # MODIFIED: Removed distance and tendon_forces from observation
            obs = np.concatenate([
                base_pos,           # 3D
                self.target_pos,    # 3D  
                base_vel,           # 3D
    ...
    def _compute_adaptive_max_steps(self, initial_distance):
        """
        NEW: Compute adaptive episode length based on initial distance
        Target: 80% of distance + 20% buffer
        """
        # Estimate steps needed to reach 80% of initial distance
        # Assume average velocity of 0.3 m/s (from action scaling)
        ...
        steps_to_80_percent = int(time_to_80_percent * 240)
    ...
    def step_wait(self):
        """Execute one step of the environment - MODIFIED reward function"""
        self.step_counts += 1
        ...
        try:
            ...
            # MODIFIED: 1. Delta-distance reward (reward for reducing distance)
            # Positive when moving closer, negative when moving away
            distance_delta = self.prev_distance[0] - distance  # Improvement in distance
            distance_reward = 10.0 * distance_delta  # Scale up the delta reward
            ...
            # MODIFIED: Termination conditions with adaptive max_steps
            dones = np.array([
    ...
    def reset_(self, dones):
        ...
        if np.any(dones):
            ...
            # NEW: Calculate initial distance and set adaptive max_steps
            if self.hand is not None:
                base_pos, _ = p.getBasePositionAndOrientation(self.hand)
...
def create_plots(csv_file):
    ...
    try:
        ...
        fig.suptitle('Base Position and Movement Analysis (OC2)', fontsize=16)
        ...
        axes[1, 0].set_title('Distance to Target vs Time (Delta-Based Reward)')
        ...
        plt.tight_layout()
        plt.savefig(plot_dir / 'base_movement_analysis_oc2.png', dpi=150, bbox_inches='tight')
        plt.close()
        print("✓ Base movement analysis plot created")
        
        # 2. Reward Components Analysis with Delta Distance
        fig, axes = plt.subplots(2, 2, figsize=(15, 10))
        fig.suptitle('Reward Components Analysis (Delta-Distance Based)', fontsize=16)
        ...
        axes[0, 1].set_title('Delta-Distance Reward (10x scaled)')
        axes[0, 1].set_xlabel('Step')
        ...
        plt.tight_layout()
        plt.savefig(plot_dir / 'reward_analysis_oc2.png', dpi=150, bbox_inches='tight')
        plt.close()
        print("✓ Reward analysis plot created")
        
        # 3. Tactile Sensor Analysis
        fig, axes = plt.subplots(2, 2, figsize=(15, 10))
        ...
        plt.tight_layout()
        plt.savefig(plot_dir / 'tactile_analysis_oc2.png', dpi=150, bbox_inches='tight')
        plt.close()
    ...
    if len(df) > 0:
        ...
        print(f"Average delta-distance reward: {df['distance_reward'].mean():.4f}")
...
if __name__ == "__main__":
    ...
    log_dir = base_training_dir / f"Run_{timestamp}_SC1_OC2_DeltaReward"
    log_dir.mkdir(parents=True, exist_ok=True)
    
    print("=" * 80)
    print(f"SC-1 OC2 (DELTA-DISTANCE REWARD) TRAINING - RUN #{run_number}")
    print("=" * 80)
    
    try:
        # Initialize WandB
        wandb.init(
            project="space-touch-sc1-oc2",
            name=f"SC1_OC2_DeltaReward_run{run_number}",
            config={
                "algorithm": "PPO",
                ...
                "max_episode_steps": "adaptive (80% distance + 20% buffer)",
                "action_space": "10D (6 base + 4 tendons)",
                "observation_space": "25D (removed distance and tendon forces)",
                "tactile_type": "binary_contact",
                "reward_type": "delta_distance",
                "reward_distance_weight": 10.0,
                "reward_accel_penalty_weight": 0.1,
                "notes": "Delta-distance reward, adaptive episode length, removed obs dimensions"
            },
            tags=["tendon-control", "binary-tactile", "delta-reward", "adaptive-episodes", "ppo", "oc2"]
        )
        
        data_logger = DataLogger(log_dir)
        
        print("\nCreating OC2 environment with delta-distance reward and adaptive episodes...")
        env = TendonAllegroReachingEnv(vis=False)
        env.set_data_logger(data_logger)
        
        callback = WandBCallback(data_logger, log_freq=100)
        
        print("Creating PPO model with OC2 configuration...")
        ...
        print("\n" + "=" * 80)
        print("SC-1 OC2 TRAINING CONFIGURATION")
        print("=" * 80)
        print(f"Log Directory: {log_dir}")
        print(f"WandB Project: space-touch-sc1-oc2")
        print(f"Target Position: {env.target_pos} (STATIC)")
        print(f"Max Steps per Episode: ADAPTIVE (80% of initial distance + 20% buffer)")
        print(f"Total Training Timesteps: 600,000")
        ...
        print("  ✓ Delta-distance reward (prev_distance - current_distance)")
        print("  ✓ Adaptive episode length based on initial distance")
        print("  ✓ Removed Euclidean distance from observation")
        print("  ✓ Removed tendon forces from observation")
        print("  ✓ 10x scaled delta-distance reward to prevent reward hacking")
        print("\nReward Components:")
        print("  - Delta-distance reward (10x weight, encourages continuous approach)")
        print("  - Tendon efficiency reward") 
        print("  - Tactile contact PENALTY (avoid accidental touch)")
        print("  - Movement efficiency penalty")
        print("  - Acceleration penalty (anti-jerk)")
        print("  - Success bonus")
        print("=" * 80)
        
        print(f"\nView live training at: https://wandb.ai/[your-username]/space-touch-sc1-oc2")
        print("\n" + "=" * 80 + "\n")
        
        print("Starting SC-1 OC2 training with WandB logging...")
        model.learn(
        ...
        model_path = log_dir / f"sc1_oc2_model_run{run_number}_{timestamp}"
        ...
        test_csv = test_data_logger.save_to_csv("sc1_oc2_test_data.csv")
        ...
        print("\n" + "=" * 80)
        print("SC-1 OC2 TEST RESULTS SUMMARY")
        print("=" * 80)
        ...
        print("\n" + "=" * 80)
        print("SC-1 OC2 TRAINING COMPLETE!")
        print("=" * 80)
        print(f"All results saved to: {log_dir}")
        print(f"Test data: {log_dir}/test_data/")
        print(f"Analysis plots: {log_dir}/test_data/plots/")
        print(f"WandB Dashboard: https://wandb.ai/[your-username]/space-touch-sc1-oc2")
        print("\nKEY IMPROVEMENTS IN OC2:")
        print("  • Delta-distance reward prevents reward hacking")
        print("  • Adaptive episode length improves training efficiency")
        print("  • Reduced observation space (25D vs 30D)")
        print("  • Hand continuously incentivized to close in on target")
        print("=" * 80)
