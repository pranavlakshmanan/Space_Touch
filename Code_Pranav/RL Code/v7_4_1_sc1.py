#!/usr/bin/env python3
"""
V7.4.1_SC1.py - Hybrid Derivative + Distance Reward Training

Key changes from V7.3:
- Adds WEAK distance reward (10.0 scale, 80:20 ratio)
...
Usage:
    python v7_4_1_sc1.py train --timesteps 50000    # Train new model
    python v7_4_1_sc1.py --resume PATH              # Resume from checkpoint
"""
...
# Import V7.4.1 reward calculator
sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
from reward_functions.v7_4_1_reward import V741RewardCalculator
...
class V741Environment(VecEnv):
    """
    V7.4.1 Soft-Capture Environment with Hybrid Reward

    Based on V7.3 with additions:
    - 32D observation space (was 28D)
    - Adds smoothed overlap + relative distance vector
    - Uses V7.4.1 hybrid reward (derivative + distance)
    ...
    def __init__(self, 
        ...
        # Observation: 32D total (V7.4.1: increased from 28D)
        ...
        # Initialize V7.4.1 hybrid reward calculator (derivative + distance)
        self.reward_calc = V741RewardCalculator({
            'object_radius': 0.05,
            'safety_margin': 0.025,
            'object_hull_points': 32,
            'hull_compute_freq': 12,  # 20Hz at 240Hz sim
            # V7.4.1 derivative reward parameters (unchanged)
            'improvement_scale': 50000.0,
            'regression_multiplier': 1.5,
            'sustain_threshold': 0.00005,  # 50 cm³
            'sustain_bonus': 2.0,
            # V7.4.1 NEW: Distance reward (80:20 ratio)
            'distance_reward_scale': 20.0,  # 10% of typical derivative reward
            'distance_reward_rate': 2.0,  # exp(-2*d)
            # V7.4.1 NEW: Overlap smoothing (EMA)
    ...
    def _get_observation(self) -> np.ndarray:
        """Build 32D observation vector (V7.4.1: was 28D)"""
        ...
        # V7.4.1: Add new observations (4D)
...
# V7.4.1: Curriculum callback REMOVED - no phases

class V741WandBCallback(BaseCallback):
    """V7.4.1 Lightweight WandB logging for hybrid reward system"""
...
def train(args):
    """Main V7.4.1 training function - Hybrid reward (80:20 ratio)"""
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    run_name = f"V7.4.1_SC1_{timestamp}"
    ...
    wandb.init(
        ...
        config={
            'version': 'V7.4.1',
            'experiment': 'hybrid_derivative_distance_80_20',
            'total_timesteps': args.timesteps,
            'algorithm': 'PPO',
            'curriculum': False,
            'observation_dim': 32,  # V7.4.1: 28 + 4 new
            'action_dim': 10,
            'position_control': True,
            'hull_points_hand': 9,
            'hull_points_object': 32,
            'overlap_method': 'bbox_fast',
            # V7.4.1 Hybrid reward parameters
            'reward_type': 'hybrid_derivative_distance',
            'derivative_distance_ratio': '80:20',
            'improvement_scale': 50000.0,
            'regression_multiplier': 1.5,
            'distance_reward_scale': 20.0,  # 80:20 ratio
        ...
        tags=['v7.4.1', 'soft-capture', 'hybrid-reward', 'derivative-distance', '80-20-ratio'],
    )

    print(f"Starting V7.4.1 training: {run_name}")
    print(f"  Total steps: {args.timesteps:,}")
    print(f"  Reward system: HYBRID (derivative + distance, 80:20 ratio)")
    ...
    env = V741Environment(vis=args.vis, max_steps=500)
    ...
    # V7.4.1: No curriculum callback
    callbacks = [
        V741WandBCallback(log_freq=480, verbose=0),
        CheckpointCallback(
            save_freq=25000,  # V7.4.1: Checkpoint every 25K for 50K runs
            save_path=str(checkpoint_dir),
            name_prefix="v7.4.1_sc1",
...
def test(args):
    ...
    print(f"="*60)
    print(f"V7.4.1 SC-1 COMPREHENSIVE TESTING (Hybrid Reward 80:20)")
    print(f"="*60)
    ...
    # Load model
    env = V741Environment(vis=args.vis, max_steps=1000)
...
def main():
    parser = argparse.ArgumentParser(description='V7.4.1 SC-1 Hybrid Reward Training (80:20)')
    ...
    if args.command == 'train':
        ...
        args.timesteps = 50000  # V7.4.1: Default 50K
        args.resume = None
        args.vis = False
        train(args)
